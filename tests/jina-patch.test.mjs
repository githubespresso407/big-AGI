...
// auto-search feature wiring
const composer = readFileSync(join(root, 'src/apps/chat/components/composer/Composer.tsx'), 'utf8');
check('composer: augment hook wired into send path', composer.includes('augmentTextWithWebSearch'));
check('composer: toggle button mounted', composer.includes('ButtonAutoSearchMemo'));
const autosearch = readFileSync(join(root, 'src/modules/jina/autosearch.ts'), 'utf8');
check('autosearch: fails open (returns userText on error)', autosearch.includes('return userText;'));
